...
- Production domain is `https://3iatlasgame.xyz`. Deploy with
  ...
  are set (see `scripts/deploy-cloudflare.mjs`). Wrangler 4 needs **Node 22+**
  (CI uses 22). The Worker is a static-asset SPA (`wrangler.jsonc`). The deploy
  script attaches Custom Domains and replaces looping apex A records.
